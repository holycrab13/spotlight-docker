...
if [ -z ${DATABUS_SPARQL_ENDPOINT+x} ]; then DATABUS_SPARQL_ENDPOINT=https://databus.dbpedia.org/repo/sparql; fi
...
then
  ...
  RESULT=`curl --data-urlencode query="$QUERY" --data-urlencode format="text/tab-separated-values" $DATABUS_SPARQL_ENDPOINT | sed 's/"//g' | grep -v "^file$" | head -n 1 `
